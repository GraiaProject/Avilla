...
class OneBot11LeaveActionPerform((m := AccountCollector["OneBot11Protocol", "OneBot11Account"]())._):
    ...
    async def leave_group(self, target: Selector):
        result = await self.account.connection.call("set_group_leave", {"group_id": int(target["group"])})
    ...
    async def disband_group(self, target: Selector):
        result = await self.account.connection.call("set_group_leave", {"group_id": int(target["group"]), "is_dismiss": True})
        if result is not None:
