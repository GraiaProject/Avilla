...
class AnnotatedParam:
    ...
    def to_param(self) -> ParamToken:
        return ParamToken((self.name,))


U_Token = Union[TextToken, ParamToken, AnnotatedParam]

ann_assign = re.compile(r"(?P<name>[^:=]+)(?P<annotation>:[^=]+)?(?P<default>=.+)?")


def parse_param(param_str: str) -> ParamToken | AnnotatedParam:
    ...
    for index, char in enumerate(string):
        if char in L_PAREN + R_PAREN:
            if char in L_PAREN:
                ...
                if paren == "[":  # CHOICE
                    token.append(TextToken(unescape(x) for x in piece.split("|")))
                elif paren == "{":  # PARAM
                    token.append(parse_param(piece))
                paren = ""
        elif char == " " and not paren:
            if char_stk:
                token.append(TextToken(pop()))
        else:
...
class MatchEntry:
    def __init__(self, tokens: list[U_Token]) -> None:
        self.nodes: list[MaybeFlag[frozenset[str]]] = [
            token.choice if isinstance(token, TextToken) else Sentinel for token in tokens
        ]
        self.tokens: list[TextToken | ParamToken] = [
            token.to_param() if isinstance(token, AnnotatedParam) else token for token in tokens
        ]
        self.params: list[ParamToken] = [token for token in self.tokens if isinstance(token, ParamToken)]
