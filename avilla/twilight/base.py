...
class MatchRegex(ChainDecorator, BaseDispatcher):
    ...
    async def beforeExecution(self, interface: DispatcherInterface[MessageReceived]):
        _mapping_str, _map = _to_mapping_str(interface.event.message.content)
    ...
    async def catch(self, interface: DispatcherInterface[MessageReceived]):
...
class RegexGroup(Decorator):
    ...
    async def __call__(self, _, interface: DispatcherInterface[MessageReceived]):
        ...
        return (
            _from_mapping_string(origin, interface.local_storage["__parser_regex_match_map__"])
    ...
    def __init__(self, template: List[Union[Type[Element], Element, str]]) -> None:
        ...
        Args:
            template (List[Union[Type[Element], Element]]): \
                匹配模板，可以为 `Element` 类或其 `Union`, `str`, `Text` 实例
        """
        ...
        for element in template:
            ...
            if isinstance(element, type):
                ...
                assert Text not in get_args(element), "Leaving Text here leads to ambiguity"  # TODO
                self.template.append(get_args(element))
            elif isinstance(element, Element) and not isinstance(element, Text):
                self.template.append(element)
            else:
                element = (
