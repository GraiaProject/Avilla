...
class RegexMatch(Match):
    ...
    def _regex_str(self) -> str:
        ...
        return f"{transform_regex(self._flags, self._src)}" f"{'?' if self.optional else ''}{self.space_policy.value}"
...
class ElementMatch(RegexMatch):
    ...
    @property
    def _src(self) -> str:
        return f"\x02\\d+_{identity(self.type)}\x03"
    ...
    def get_help(
        ...
        if self.match_ref[RegexMatch]:
            ...
            for match in self.match_ref[RegexMatch]:
                if match._help:
                    formatter.add_text(f"""{ f"{match.dest}{sep}" if dest and match.dest else ""}{match._help}""")
...
class Twilight(Generic[T_Sparkle], BaseDispatcher):
    ...
    def generate(self, chain: MessageChain, storage: Optional[Dict[str, Any]] = None) -> T_Sparkle:
        ...
        """
        mapping_str, elem_mapping = _to_mapping_str(chain, **self.map_param)
    ...
    async def beforeExecution(self, interface: DispatcherInterface):
        ...
        if isinstance(self.preprocessor, ChainDecorator):
            ...
            chain = await interface.lookup_by_directly(DeriveDispatcher(), "twilight_derive", self.preprocessor, None)
        else:
    ...
    async def catch(self, interface: DispatcherInterface):
        ...
        if interface.name in sparkle.res:
            ...
            if generic_isinstance(result.origin, interface.annotation):
                ...
                logger.warning(f"Please use {interface.name}: {result.__class__.__qualname__} to get origin attribute!")
                interface.stop()
...
class Help(Decorator, Generic[T]):
    ...
    def __init__(self, formatter: Optional[Callable[[str, DecoratorInterface], Union[Awaitable[T], T]]] = None) -> None:
        """
        Args:
    ...
    async def target(self, i: DecoratorInterface) -> T:
        ...
        help_string: str = twilight.matcher.get_help(**(twilight.help_data or {}))  # type: ignore
        if self.formatter:
