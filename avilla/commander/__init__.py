...
class Arg(ParamDesc):
    ...
    def gen_model(self, validators: Iterable[Callable]) -> None:
        ...
        if (
            ...
            and not issubclass(self.type, InnerTypes)
        ):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            [
                ...
                *resolve_dispatchers_mixin(cast(List[T_Dispatcher], dispatchers or [])),
            ],
            decorators or [],
        )
    ...
    def get_data(
        ...
        for arg in set(self.pattern.arg_map.values()):
            ...
            if arg.nargs:
                ...
                if not issubclass(arg.type, BaseModel) or issubclass(arg.type, InnerTypes):
...
class Commander:
        ...
        for (t_type, tokens) in command_tokens:
            if t_type in {CommandToken.TEXT, CommandToken.CHOICE}:
                ...
                for param_name in tokens:
                    assert not param_name in placeholder_set, "Duplicated parameter slot!"
    ...
    async def execute(self, chain: MessageChain):
        ...
        mapping_str, elem_m = chain_to_mapping_str(chain)
            ...
            with suppress(IndexError, MismatchError, ValueError, RequirementCrashed, ExecutionStop):
                while text_index < len(text_list):
                    ...
                    if text in pattern.arg_map:  # Arg handle
                        ...
                        arg_data[text] = [
                            chain_from_mapping_string(t, elem_m)
                        ...
                        if isinstance(tokens, list):
                            if pattern.last_type is CommandPattern.ELast.WILDCARD and token_index == len(
                                ...
                                wildcard_list.append(chain_from_mapping_string(text, elem_m))
                                token_index -= 1
                            for slot in tokens:
                                slot_data[slot] = chain_from_mapping_string(text, elem_m)
