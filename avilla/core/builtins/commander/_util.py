...
class TextFrag:
    ...
    def __init__(self, choice: Iterable[str] | str) -> None:
        object.__setattr__(self, "choice", frozenset((choice,) if isinstance(choice, str) else choice))
...
class AnnotatedParam:
    ...
    def __repr__(self) -> str:
        return f"AnnotatedParam({'...' if self.wildcard else ''}{self.name}: {self.annotation} = {self.default})"

    def to_param(self) -> ParamFrag:
        return ParamFrag((self.name,))


U_Token = Union[TextFrag, ParamFrag, AnnotatedParam]

ann_assign = re.compile(r"(?P<name>[^:=]+)(?P<annotation>:[^=]+)?(?P<default>=.+)?")


def parse_param(param_str: str) -> ParamFrag | AnnotatedParam:
    ...
    if not wildcard and not any(extra):
        return ParamFrag(names)
    assert len(names) == 1, f"Invalid param: {param_str}"
    return AnnotatedParam(names[0], wildcard, *(unescape(s).strip().lstrip(":=").strip() if s else None for s in extra))
...
def tokenize(string: str) -> list[U_Token]:
    """将字符串转义化, 并处理为 TextFrag, ParamFrag 两种 token
    ...
    for index, char in enumerate(string):
        if char in L_PAREN + R_PAREN:
            if char in L_PAREN:
                ...
                if paren == "[":  # CHOICE
                    token.append(TextFrag(unescape(x) for x in piece.split("|")))
                elif paren == "{":  # PARAM
                    token.append(parse_param(piece))
                paren = ""
        elif char == " " and not paren:
            if char_stk:
                token.append(TextFrag(pop()))
        else:
...
class MatchEntry:
    def __init__(self, tokens: list[U_Token]) -> None:
        self.nodes: list[MaybeFlag[frozenset[str]]] = [
            token.choice if isinstance(token, TextFrag) else Sentinel for token in tokens
        ]
        self.tokens: list[TextFrag | ParamFrag] = [
            token.to_param() if isinstance(token, AnnotatedParam) else token for token in tokens
        ]
        self.params: list[ParamFrag] = [token for token in self.tokens if isinstance(token, ParamFrag)]
...
def split(chain: MessageChain) -> ChainContentList:
    ...
    for elem in chain.content:
        if elem.__class__.__name__ == "Quote":
            continue
...
def const_fn(value: T) -> Callable[[], T]:
    return lambda: value


def gen_subclass(cls: type[T]) -> Iterable[type[T]]:
    yield cls
...
def resolve_dispatchers_mixin(dispatchers: Iterable[T_Dispatcher]) -> list[T_Dispatcher]:
    ...
    for d in dispatchers:
        res.extend(dispatcher_mixin_handler(d))
    return res


GRAIA_AFFILIATED = ["ichika", "graia", "avilla"]


def graia_affiliated(cls: type) -> bool:
    return any(cls.__module__.startswith(x) for x in GRAIA_AFFILIATED)
