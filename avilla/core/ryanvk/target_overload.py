...
from avilla.core.flywheel import FnOverload, BaseCollector
...
class LookupBranch:
    ...
    bind: dict[tuple[BaseCollector, Callable], None] = field(default_factory=dict)
...
class TargetOverload(FnOverload[TargetOverloadSignature, tuple[str, dict[str, FollowsPredicater]], Selector]):
    ...
    def collect(self, scope: dict, signature: TargetOverloadSignature) -> dict[tuple[BaseCollector, Any], None]:
    ...
    def harvest(self, scope: dict, value: Selector) -> dict[tuple[BaseCollector, Any], None]:
    ...
    def access(self, scope: dict, signature: TargetOverloadSignature) -> dict[tuple[BaseCollector, Any], None] | None:
        pattern_items = _parse_follows(signature.order)
        if not pattern_items:
            raise ValueError("invalid target pattern")
