from datetime import datetime, timedelta
from typing import Any, Dict, Generic, List, Literal, TypeVar, Union, overload

from avilla.core.execution import Execution as Execution
from avilla.core.protocol import BaseProtocol as BaseProtocol
from avilla.core.selectors import entity as entity
from avilla.core.selectors import mainline
from avilla.core.selectors import mainline as mainline
from avilla.core.selectors import resource as resource
from avilla.core.typing import TExecutionMiddleware as TExecutionMiddleware

class ExecutorWrapper:
    relationship: "Relationship"
    execution: Execution
    middlewares: List[TExecutionMiddleware]
    def __init__(self, relationship: "Relationship") -> None: ...
    def __await__(self): ...
    def execute(self, execution: Execution): ...
    __call__: Any
    def to(self, target: Union[entity, mainline]): ...
    def period(self, period: timedelta): ...
    def use(self, middleware: TExecutionMiddleware): ...

class _RelationshipMetaWrapper:
    relationship: "Relationship"
    def __init__(self, relationship: "Relationship") -> None: ...
    async def get(self, metakey: str) -> Any: ...
    async def set(self, metakey: str, value: Any) -> None: ...
    async def reset(self, metakey: str) -> None: ...
    async def prev(self, metakey: str) -> Any: ...
    async def next(self, metakey: str) -> Any: ...
    async def push(self, metakey: str, value: Any) -> None: ...
    async def pop(self, metakey: str, index: int) -> Any: ...
    async def add(self, metakey: str, value: Any) -> None: ...
    async def remove(self, metakey: str, value: Any) -> None: ...

M = TypeVar("M", bound=_RelationshipMetaWrapper)

class Relationship(Generic[M]):
    ctx: entity
    mainline: mainline
    self: entity
    protocol: "BaseProtocol"
    _middlewares: List[TExecutionMiddleware]
    def __init__(
        self,
        protocol: BaseProtocol,
        ctx: entity,
        mainline: mainline,
        current_self: entity,
        middlewares: List[TExecutionMiddleware] = ...,
    ) -> None: ...
    @property
    def current(self) -> entity: ...
    @property
    def meta(self) -> M: ...
    @property
    def exec(self): ...
    def has_ability(self, ability: str) -> bool: ...

class CoreSupport(_RelationshipMetaWrapper):
    #
    # Mainline Properties
    #
    @overload
    async def get(self, metakey: Literal["mainline.name"]) -> str:
        "mainline.name -> str: Avilla Protocol 中 Mainline 的名称，在其他实现中通常被称为 `群组名称`， `频道名称` 等。"
        ...
    @overload
    async def set(self, metakey: Literal["mainline.name"], value: str) -> None:
        "mainline.name -> str: Avilla Protocol 中 Mainline 的名称，在其他实现中通常被称为 `群组名称`， `频道名称` 等。"
        ...
    @overload
    async def get(self, metakey: Literal["mainline.avatar"]) -> resource:
        "mainline.avatar -> resource: Avilla Protocol 中 Mainline 的头图，在其他实现中通常被称为 `群组头像`，`频道头像` 等。"
        ...
    @overload
    async def get(self, metakey: Literal["mainline.description"]) -> str:
        "mainline.description -> str: Avilla Protocol 中 Mainline 的描述，在其他实现中通常被称为 `群组描述`， `频道描述` 等。"
        ...
    @overload
    async def set(self, metakey: Literal["mainline.description"], value: str) -> None:
        "mainline.description -> str: Avilla Protocol 中 Mainline 的描述，在其他实现中通常被称为 `群组描述`， `频道描述` 等。"
        ...
    @overload
    async def reset(self, metakey: Literal["mainline.description"]) -> None:
        "mainline.description -> str: Avilla Protocol 中 Mainline 的描述，在其他实现中通常被称为 `群组描述`， `频道描述` 等。"
        ...
    @overload
    async def get(self, metakey: Literal["mainline.max_count"]) -> int:
        "mainline.max_count -> int: Avilla Protocol 中 Mainline 所能容纳的最大人数，在其他实现中通常被称为 `群组人数上限`， `频道人数上限` 等。"
        ...
    @overload
    async def get(self, metakey: Literal["mainline.current_count"]) -> int:
        "mainline.current_count -> int: Avilla Protocol 中 Mainline 的当前人数，在其他实现中通常被称为 `群组人数`， `频道人数` 等。"
        ...
    #
    # Member Properties
    #
    @overload
    async def get(self, metakey: Literal["member.name"]) -> str:
        "member.name -> str: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 的名称，在其他实现中通常被称为 `用户名`， `昵称` 等，在部分实现中，`member.nickname` 是 `member.name` 的 alias。"
        ...
    @overload
    async def get(self, metakey: Literal["member.avatar"]) -> resource: ...
        "member.avatar -> resource: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 的头图，在其他实现中通常被称为 `用户头像`， `昵称头像` 等。"
        ...
    @overload
    async def get(self, metakey: Literal["member.nickname"]) -> str:
        "member.nickname -> str: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 的昵称，在其他实现中通常被称为 `用户名`， `昵称` 等。"
        ...
    @overload
    async def set(self, metakey: Literal["member.nickname"], value: str) -> None:
        "member.nickname -> str: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 的昵称，在其他实现中通常被称为 `用户名`， `昵称` 等。"
        ...
    @overload
    async def reset(self, metakey: Literal["member.nickname"]) -> None:
        "member.nickname -> str: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 的昵称，在其他实现中通常被称为 `用户名`， `昵称` 等。"
        ...
    @overload
    async def get(self, metakey: Literal["member.budget"]) -> str:
        "member.budget -> str: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 被特权用户赋予的特殊头衔。"
        ...
    @overload
    async def set(self, metakey: Literal["member.budget"], value: str) -> None:
        "member.budget -> str: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 被特权用户赋予的特殊头衔。"
        ...

    @overload
    async def reset(self, metakey: Literal["member.budget"]) -> None:
        "member.budget -> str: Avilla Protocol 中, Mainline 的组成部分，即成员 Member 被特权用户赋予的特殊头衔。"
        ...
    @overload
    async def get(self, metakey: Literal["member.muted"]) -> bool:
        "member.muted -> bool: 指示成员 Member 是否已被其它特权用户禁止发言，这通常代表执行 MessageSend 不能正常使用。"
        ...
    @overload
    async def get(self, metakey: Literal["member.mute_period"]) -> Union[timedelta, None]:
        "member.mute_period -> timedelta: 指示成员 Member 的禁言时长，如果为 None 表示没有禁言。"
        ...
    @overload
    async def get(self, metakey: Literal["member.joined_at"]) -> datetime:
        "member.joined_at -> datetime: 成员 Member 的加入时间。"
        ...
    @overload
    async def get(self, metakey: Literal["member.last_active_at"]) -> datetime:
        "member.last_active_at -> datetime: 成员 Member 的最后活动时间，通常是最近一次发言的时间。"
        ...
    #
    # Contact(like friend, worker etc.) Properties
    #
    @overload
    async def get(self, metakey: Literal["contact.name"]) -> str: ...
    @overload
    async def get(self, metakey: Literal["contact.nickname"]) -> str: ...
    @overload
    async def get(self, metakey: Literal["contact.avatar"]) -> resource: ...
    #
    # Request Properties
    #
    @overload
    async def get(self, metakey: Literal["request.comment"]) -> str: ...
    @overload
    async def get(self, metakey: Literal["request.reason"]) -> str: ...
    @overload
    async def get(self, metakey: Literal["request.has_question"]) -> bool: ...
    @overload
    async def get(self, metakey: Literal["request.questions"]) -> Dict[int, str]: ...
    @overload
    async def get(self, metakey: Literal["request.answers"]) -> Dict[int, str]: ...
    @overload
    async def get(self, metakey: Literal["request.qa"]) -> Dict[str, str]: ...
    #
    # Self Properties
    #
    @overload
    async def get(self, metakey: Literal["self.name"]) -> str: ...
    @overload
    async def set(self, metakey: Literal["self.name"], value: str) -> None: ...
    @overload
    async def get(self, metakey: Literal["self.nickname"]) -> str: ...
    @overload
    async def set(self, metakey: Literal["self.nickname"], value: str) -> None: ...
    @overload
    async def get(self, metakey: Literal["self.budget"]) -> str: ...
    @overload
    async def set(self, metakey: Literal["self.budget"], value: str) -> None: ...
    @overload
    async def reset(self, metakey: Literal["self.budget"]) -> bool: ...
    @overload
    async def get(self, metakey: Literal["self.muted"]) -> bool: ...
    @overload
    async def get(self, metakey: Literal["self.mute_period"]) -> Union[datetime, None]: ...
    @overload
    async def get(self, metakey: Literal["self.joined_at"]) -> datetime: ...
    @overload
    async def get(self, metakey: Literal["self.last_active_at"]) -> datetime: ...
    #
    # necessary overrides
    #
    @overload
    async def get(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def set(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def reset(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def prev(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def next(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def push(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def pop(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def add(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def remove(self, metakey: Literal["$python.typing.stubfile.overload"]) -> None: ...
    @overload
    async def get(self, metakey: ...) -> Any: ...
    @overload
    async def set(self, metakey: ..., value: Any) -> None: ...
    @overload
    async def reset(self, metakey: ...) -> None: ...
    @overload
    async def prev(self, metakey: ...) -> Any: ...
    @overload
    async def next(self, metakey: ...) -> Any: ...
    @overload
    async def push(self, metakey: ..., value: ...) -> None: ...
    @overload
    async def pop(self, metakey: ..., index: ...) -> Any: ...
    @overload
    async def add(self, metakey: ..., value: ...) -> None: ...
    @overload
    async def remove(self, metakey: ..., value: ...) -> None: ...  # type: ignore
