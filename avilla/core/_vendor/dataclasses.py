...
def _field_init(f, frozen, globals, self_name, slots):
    ...
    return _field_assign(frozen, f.name, value, self_name)  # type: ignore
...
def _init_param(f):
    ...
    return f"{f.name}:_type_{f.name}{default}"  # type: ignore
...
def _init_fn(fields, std_fields, kw_only_fields, frozen, has_post_init, self_name, globals, slots):
    ...
    return _create_fn(
        "__init__", [self_name] + _init_params, body_lines, locals=locals, globals=globals, return_type=None
    )
...
def _get_field(cls, a_name, a_type, default_kw_only):
    ...
    f._field_type = _FIELD  # type: ignore
    ...
    if typing:
        if _is_classvar(a_type, typing) or (
            ...
            f._field_type = _FIELD_CLASSVAR  # type: ignore
    ...
    if f._field_type is _FIELD:
        ...
        if _is_initvar(a_type, dataclasses) or (
            ...
            f._field_type = _FIELD_INITVAR  # type: ignore
...
def _process_class(cls, init, repr, eq, order, unsafe_hash, frozen, match_args, kw_only, slots, weakref_slot):
    ...
    for name, type in cls_annotations.items():
        ...
        if _is_kw_only(type, dataclasses) or (
            isinstance(type, str) and _is_type(type, cls, dataclasses, dataclasses.KW_ONLY, _is_kw_only)
        ):
    ...
    hash_action = _hash_action[bool(unsafe_hash), bool(eq), bool(frozen), has_explicit_hash]  # type: ignore
    if hash_action:
