...
class FileDirectoryCreate(Capability):
    @Fn.complex({TargetOverload(): ["target"]})
    async def create(self, target: Selector, name: str, parent: str | None = None) -> Selector: ...
...
class FileMove(Capability):
    @Fn.complex({TargetOverload(): ["file"]})
    async def move(self, file: Selector, to: Selector) -> None: ...


class FileRename(Capability):
    @Fn.complex({TargetOverload(): ["file"]})
    async def rename(self, file: Selector, name: str) -> None: ...
