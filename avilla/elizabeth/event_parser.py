    ...
    async def group_message(self, protocol: ElizabethProtocol, account: AccountSelector, raw: dict):
        ...
        return MessageReceived(
            message=Message(
                ...
                sender=Selector().land(protocol.land.name).group(raw['sender']['group']['id']).member(raw['sender']['id']),
                ...
                reply=Selector().land(protocol.land.name).group(raw['sender']['group']['id']).message(str(quote.id)) if quote is not None else None,
            ),
            account=account
        )
