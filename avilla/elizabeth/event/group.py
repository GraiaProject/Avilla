...
from avilla.core.event import MetadataModified, Op, RelationshipCreated, RelationshipDestroyed, Update
from avilla.spec.core.privilege.metadata import MuteInfo
from avilla.spec.core.privilege.skeleton import MuteAllTrait, MuteTrait
...
async def group_name_changed(protocol: ElizabethProtocol, account: ElizabethAccount, raw: dict[str, Any]):
    ...
    return MetadataModified(
        context=context,
        endpoint=group,
        modifies=[Op(NickTrait.set_name, {
...
async def group_muteall(protocol: ElizabethProtocol, account: ElizabethAccount, raw: dict[str, Any]):
    ...
    return MetadataModified(
        context=context,
        endpoint=group,
        modifies=[Op(MuteAllTrait.mute_all, {
